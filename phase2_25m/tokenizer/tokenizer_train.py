...
training. Settings are locked to the confirmed production configuration:
    ...
    vocab size     : fixed at VOCAB_SIZE. The corpus is expected to
                      naturally support this size -- NO post-hoc padding
                      is applied. Padding to force an exact round number
                      would waste embedding parameters on inert placeholder
                      tokens that are never produced during real encoding,
                      which isn't worth it at this model scale. Whatever
                      the real corpus achieves IS the final, honest vocab
                      size.
    context length : 1400  (fixed by explicit choice for this run --
                             informational here, actually enforced later
                             during dataset batching / model config.)
    special tokens : none (no custom user-defined tokens)
...
each one down to a single canonical "___" marker.
...
TRAIN_FILE = Path("train1.jsonl")
OUTPUT_DIR = Path("tokenizer")
MODEL_PREFIX = "slm_tokenizer_11000"

# Fixed target vocab size. No custom tokens, so no extra pieces to add on
# top of the learned vocabulary + reserved special tokens.
VOCAB_SIZE = 11000
...
def clean_placeholders(text):
    """Collapse placeholder/junk runs to a single canonical token."""
    n_runs = 0
    n_chars = 0
    ...
    cleaned = PLACEHOLDER_RUN_RE.sub(_replace, text)
...
def train_one(corpus_txt, vocab_size):
    requested_vocab_size = vocab_size
    model_path_prefix = str(OUTPUT_DIR / MODEL_PREFIX)
...
Answer: Newton""",
        """Question: A car travels 60 km in 2 hours.
...
def main():
    ...
    if not sp.Load(model_path_prefix + ".model"):
    ...
    print(f"    = {actual_learned_pieces:,} learned pieces "
          ...
          f"+ {len(SPECIAL_TOKENS)} custom tokens")
